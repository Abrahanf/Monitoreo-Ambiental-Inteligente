...
static const char *TAG = "DHT11_2";
#define I2C_MASTER_FREQ_HZ 100000

// Constantes y variables del wifi mesh
...
void dht_task(void *pvParameter)
{
    ...
    // Define el ID de este nodo (Ej: Nodo 2)
    int nodo_id = 3; 

    if (dht11_read(DHT11_GPIO, &temperature, &humidity) == 0) {
        
        // 1. Mostrar en consola (Para ti)
        ESP_LOGI(TAG, "Lectura: Temp=%d C, Hum=%d %%", temperature, humidity);

        // 2. Preparar el JSON
        // Creamos un buffer temporal. 128 bytes es suficiente para este mensaje.
        char json_payload[128]; 
        
        // Construimos el string con formato JSON estricto
        // Nota: Enviamos "co2": 0 porque este sensor no mide gas
        snprintf(json_payload, sizeof(json_payload), 
                "{\"nodo_id\": %d, \"temp\": %d, \"hum\": %d, \"co2\": 0}", 
                nodo_id, temperature, humidity);

        // 3. Publicar al tema GLOBAL que escucha el Docker
        // Nota: Cambia "topic/..." por "environmental/measurements"
        mqtt_app_publish("environmental/measurements", json_payload);

        } else {
            ESP_LOGE(TAG, "No se puede leer la data del sensor");
        }
