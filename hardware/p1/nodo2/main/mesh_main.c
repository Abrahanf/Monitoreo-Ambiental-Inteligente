...
void esp_mesh_mqtt_task(void *arg)
{
    ...
    while (is_running) {
        ...
        mqtt_app_publish("/topic/ip_mesh_nodo2", print);
        free(print);
        if (esp_mesh_is_root()) {
...
void dht_task(void *pvParameter)
{
    ...
    // Configurar ADC1 canal 6 (GPIO 34)
    adc1_config_width(ADC_WIDTH_BIT_12); // Resolución de 12 bits (0-4095)
    adc1_config_channel_atten(ADC1_CHANNEL_6, ADC_ATTEN_DB_11); // Mayor rango de voltaje (~0 - 3.3V)
  
    while (1) {
        char *data_sensor;
        int val = adc1_get_raw(ADC1_CHANNEL_6); // Leer valor crudo
        float voltage = val * (3.3 / 4095);     // Convertir a voltaje (opcional)
        
        ESP_LOGI(TAG, "ADC Voltaje: %.2f V", voltage);
        asprintf(&data_sensor,"ADC Voltaje: %.2f V", voltage);
        mqtt_app_publish("/topic/nodo2/potenciometro", data_sensor);
        free(data_sensor);
