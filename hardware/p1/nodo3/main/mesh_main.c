...
void esp_mesh_mqtt_task(void *arg)
{
    ...
    while (is_running) {
        ...
        mqtt_app_publish("/topic/ip_mesh_nodo3", print);
        free(print);
        if (esp_mesh_is_root()) {
...
void dht_task(void *pvParameter)
{
    ...
    if (dht_read_data(DHT_TYPE_DHT11, DHT11_GPIO, &humidity, &temperature) == ESP_OK) {
            ESP_LOGI(TAG, "Temperature: %d°C, Humidity: %d%%", temperature/10, humidity/10);
            asprintf(&data_sensor,"Temperature: %d°C, Humidity: %d%%", temperature/10, humidity/10);
            mqtt_app_publish("/topic/nodo3/temp_humedad", data_sensor);
            free(data_sensor);
